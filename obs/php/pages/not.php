			...
			<form method="post">
				<table class="table table-dark bg-dark">
					...
					<tbody class="text-center">
							...
							<td class="">
								<select name='lesson' class="">
									...
									foreach ($lessonsrows as $lesson) {
										echo "<option value='".$lesson->tcNum."'>".$lesson->DersAdi."</option>" ; 
									}
									?>
								</select>
							</td>
						...
						<tr>
							<td colspan="8">
								<input type='hidden' name='tcNum' value="<?php echo $OgrenciID ?>">
...
if(isset($_POST['notekle'])){
	...
	$checkgrade = $addgrade->getrow("INSERT INTO Ogrenci_Ders_Notlari (tcNum, DersID, $snvTuru) VALUES (?, ?, ?)",array($ogrenciID,$DersID,$notu));
